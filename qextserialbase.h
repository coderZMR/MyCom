...
struct PortSettings 
{
    BaudRateType BaudRate;  // 设置传输速率类型
    DataBitsType DataBits;  // 设置数据位长度类型
    ParityType Parity;      // 设置奇偶校验
    StopBitsType StopBits;  // 设置停止位
    FlowType FlowControl;   // 设置数据流控制，解决数据丢失问题
    long Timeout_Millisec;  // 设置延时大小，作用：规定上位机等待多长时间再去读串口数据
};
